...
- **Address/cadastral** (`address-lookup.ts`): DAR (Danmarks Adresseregister) lookup, no auth. DAWA (`dawa.aws.dk`) is being retired ~2026-08-17; unlike legacy DAWA's single bundled endpoint, DAR (id_lokalid/matrikelnr/ejerlav) and zone status (byzone/landzone/sommerhusomraade) are separate Datafordeler services, so they're fetched independently (`fetchDarAddress`/`fetchZone`, each with its own try/catch) — a failure in one can't blank out the other's result, and the lookup only fails outright when both come back empty. Verify both live response shapes before flipping `ADDRESS_LOOKUP_MOCK_MODE=false`. Resolves `properties.id_lokalid`/`matrikelnr`/`ejerlav`/`zone` for every property during ingest (migration 010).
...
- **BBR** (`bbr.ts`): Datafordeler's entity-based BBR GraphQL service (`https://graphql.datafordeler.dk/BBR/<version>`, per Klimadatastyrelsen's "Transitionsguide for GraphQL-tjenester" v2.3), requires a `DATAFORDELER_API_KEY` (still pending — same blocker OIS/Vejdirektoratet enrichment awaits). Auth is an API key query parameter — the old REST-style `DATAFORDELER_USERNAME`/`DATAFORDELER_PASSWORD` "tjenestebruger" login only works for fetching the GraphQL schema, not for querying entity data, so it can't be reused here. Fetches a full building-level record — `byg026Opførelsesår` (year built), `byg027OmTilbygningsår` (renovation year), `byg038SamletBygningsareal` (area), `byg021BygningensAnvendelse` (building use code), `byg054AntalEtager` (floors), `byg033Tagdækningsmateriale` (roof material), `byg032YdervæggensMateriale` (wall material), and `byg056Varmeinstallation` (heating installation) — rather than just heating alone; `enrich.ts` prefers these real values over the listing-derived guesses (`listing.building_year`/`sqm`/`property_type`) whenever the BBR lookup succeeds, and the property detail page (`apps/web/src/routes/property/[id].tsx`) now surfaces floors/roof/wall material alongside the existing energy-label/renovation stats. `oilTankRisk` still upgrades from the `buildingYear < 1970` heuristic to the real `byg056Varmeinstallation` field the same way as before. Confirmed against Datafordeler's own "GraphQL (BBR)" doc that the newest version is **v3** (bumped from this repo's earlier `v1` default) and the top-level entity is `Bygning` (not `BBR_Bygning` as queried) — but that doc doesn't list field-level names (`byg026Opførelsesår` etc., which live in the separate objekttypekatalog), so `BUILDING_QUERY`'s entity name and field names are both still unverified against the live v3 schema (`.../BBR/v3/schema?apiKey=...`) before flipping `BBR_MOCK_MODE=false`; falls back to the heuristic/listing-derived values when BBR is unavailable.
...
- **Ejendomsvurdering** (`ejendomsvurdering.ts`): Datafordeler's VUR (offentlig ejendomsvurdering) GraphQL service (`https://graphql.datafordeler.dk/VUR/<version>`), confirmed live against Datafordeler's own "GraphQL (VUR)" doc (endpoint pattern, v2, entities `Ejendomsvurdering`/`Vurderingsejendom`) — same `DATAFORDELER_API_KEY`, no separate credential. Resolves `enrichments.public_valuation` (migration 013): assessed property value, assessed land value, and valuation year — a real assessed value is directly comparable to listing price. **Two things remain unverified**: the doc states `Vurderingsejendom` is keyed by BFE-nummer, which nothing in this codebase resolves today, so this queries by `matrikelnr`/`ejerlav` instead on the unconfirmed assumption that `Jordstykke` has a queryable relation back to its valuation; and the exact field names (`ejendomsværdi`/`grundværdi`/`vurderingsår`) are guesses, since the field-level objekttypekatalog wasn't available. Confirm both against the live schema (`.../VUR/v2/schema?apiKey=...`) before flipping `EJENDOMSVURDERING_MOCK_MODE=false` — if BFE turns out to be required, a BFE lookup needs to be added to `address-lookup.ts` or `matrikel.ts` first.
