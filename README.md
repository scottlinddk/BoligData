...
- **BBR** (`bbr.ts`): Datafordeler BBR/DAR, requires `DATAFORDELER_USERNAME`/`DATAFORDELER_PASSWORD` credentials (still pending — same blocker OIS/Vejdirektoratet enrichment awaits). Once live, upgrades `oilTankRisk` from the `buildingYear < 1970` heuristic to BBR's real `varmeinstallation` (heating installation) field; falls back to the heuristic when BBR is unavailable.

**Explicitly out of scope for now**: Støjbelastning (noise) and Kloakseparering (sewer separation) checklist items stay on the deterministic mock derivation indefinitely — neither has an integrated data source. Noise would need Miljøstyrelsens støjkort (no confirmed open API); sewer separation needs per-municipality spildevandsplan data (no unified API across Denmark's ~98 municipalities).
...
4. Done — a live trial (`CRAWL_MOCK_MODE=false`) confirmed Boligsiden's field mappings and ingested a real listing end-to-end; Boliga is blocked (`HTTP 403` from Vercel's IPs, see "Data ingest status" above) and disabled via `CRAWL_SOURCES=boligsiden` until that's resolved. Register for Datafordeler credentials to unlock real BBR enrichment (`BBR_MOCK_MODE=false`, see "Data ingest status" above).
5. Apply migration `packages/supabase/migrations/010_cadastral_fields.sql` (adds `properties.id_lokalid`/`matrikelnr`/`ejerlav`/`zone`) to the Supabase project.
