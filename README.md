...
4. Done — a live trial (`CRAWL_MOCK_MODE=false`) confirmed Boligsiden's field mappings and ingested a real listing end-to-end; Boliga is blocked (`HTTP 403` from Vercel's IPs, see "Data ingest status" above) and disabled via `CRAWL_SOURCES=boligsiden` until that's resolved. Register for Datafordeler credentials to unlock real BBR/OIS enrichment (`ENRICH_MOCK_MODE=false`).
